...
pub(crate) struct SymbolWalker {}

impl SymbolWalker {
    pub(crate) fn walk(node: &ASTNode<Program>) {
        let Program { source_elements } = node.ctx_ref();
        if let Some(source_elements) = source_elements {
            walk_source_elements(source_elements);
        }
    }
}

fn walk_source_elements(source_elements: &ASTNode<SourceElements>) {
...
fn walk_stat(stat: &ASTNode<Stat>) {
    match stat.ctx_ref() {
